...
void displayCurve(void);//显示函数,需要显示时调用此函数
...
void drawCurve(int argc, char** argv)
{
    P = new point[MAXPOINTS];//控制点数组指针

    cout << "按下\"ESC\"键退出。按下\"R\"键重新绘制\r" << endl;
    cout << "最多添加20个点" << endl;
    ...
    glutInitWindowSize(1500, 700);//初始化窗口宽度和高度
    glutCreateWindow("Computer Graphics");//创建窗口，"Bézier curve"为窗口名字
    ...
    glutDisplayFunc(displayCurve);//当窗口内容被绘制时调用displayCurve函数
...
void Draw_Bézier_curve(int n, point* p, int m)
{
    int i, j;
    float t;
    ...
    for (i = 0; i <= m; i++)
    {
        ...
        {
            Bézier_curve_pointx += C[j] * pow(1 - t, control_points_n - j - 1) * pow(t, j) * p[j].x;
            Bézier_curve_pointy += C[j] * pow(1 - t, control_points_n - j - 1) * pow(t, j) * p[j].y;
        }
...
void displayCurve(void)
{
    ...
    {
        Draw_Bézier_curve(control_points_n - 1, P, 200);
    }
